...
NeverLose.Flags = {};
NeverLose.AccentColor = Color3.fromRGB(118, 185, 0);
NeverLose.AccentColorSignal = nil; -- se inicializa después de CreateSignal
...
if getcustomasset then
	...
	pcall(function()
		if not isfile(dir..'/'..'logo.png') then
			...
			pcall(function() writefile(dir..'/'..'logo.png' , byte); end);
			task.wait();
		end;
	...
	pcall(function()
		if not isfile(dir..'/'..'saturation_value_gradient.png') then
			...
			pcall(function() writefile(dir..'/'..'saturation_value_gradient.png' , byte); end);
			task.wait();
		end;
...
-- Función para agregar iconos personalizados
...
-- Inicializar AccentColorSignal ahora que CreateSignal está disponible
NeverLose.AccentColorSignal = NeverLose:CreateSignal(Color3.fromRGB(118, 185, 0));

NeverLose.SetIconMode = LPH_NO_VIRTUALIZE(function(self , Label: TextLabel , Icon: string)
	-- Verificar si es un asset ID (rbxassetid://)
	if string.find(Icon, "rbxassetid://") then
		-- Es un asset ID, crear ImageLabel en lugar de TextLabel
		...
		-- Ocultar el TextLabel original
		Label.Visible = false
		
		return ImageIcon
	else
		-- Es un icono de texto normal
...
function NeverLose:CreateColorPicker(HandleFrame: Frame)
	...
	ColorPickerLib.SetRender = LPH_NO_VIRTUALIZE(function(value)
		if value then
			-- Si el color picker queda fuera de pantalla abajo, abrirlo hacia arriba
			local screenH = NeverLose.ScreenGui.AbsoluteSize.Y
			local pickerH = 240
			local posY = HandleFrame.AbsolutePosition.Y + 75
			if posY + pickerH > screenH then
				posY = HandleFrame.AbsolutePosition.Y - pickerH - 5
			end
			ColorPickerHandler.Position = UDim2.new(0, HandleFrame.AbsolutePosition.X + 20, 0, posY);
...
function NeverLose:RegisiterHandler(Handler: Frame , Signal)
	...
	function handle:AddToggle(Config)
		...
		end);		ToggleLib.SetVisible = LPH_NO_VIRTUALIZE(function(value)
			if value then
		...
		-- Suscribirse al AccentColor signal para actualizar en tiempo real
		NeverLose:AddSignal(NeverLose.AccentColorSignal:Connect(function()
			if Config.Default then
				Toggle.BackgroundColor3 = NeverLose.AccentColor
			end
		end));
	...
	function handle:AddKeybind(Config)
		...
		-- Suscribirse al AccentColor signal
		NeverLose:AddSignal(NeverLose.AccentColorSignal:Connect(function()
			Keybind.BackgroundColor3 = NeverLose.AccentColor
			UIStroke.Color = NeverLose.AccentColor
		end));
	...
	function handle:AddToggleKeybind(Config)
		...
		-- Toggle (redondito, igual al AddToggle normal)
		local Toggle = Instance.new("Frame")
		local ToggleUICorner = Instance.new("UICorner")
		local Circle = Instance.new("Frame")
		local CircleUICorner = Instance.new("UICorner")

		-- Keybind (igual al AddKeybind normal)
		local Keybind = Instance.new("Frame")
		...
		-- Keybind setup (mismo estilo que AddKeybind)
		Keybind.Name = NeverLose.RandomString();
		Keybind.Parent = Handler
		Keybind.BackgroundColor3 = NeverLose.AccentColor
		Keybind.BackgroundTransparency = 0.5
		...
		ToggleKeybindLib.SetVisible = LPH_NO_VIRTUALIZE(function(value)
			if value then
				...
				NeverLose.PlayAnimate(Circle, SlowyTween, { BackgroundTransparency = 1 })
		...
		-- Keybind click (mismo comportamiento que AddKeybind)
		NeverLose:CreateInput(Keybind, function()
		...
		-- Monitorear tecla: togglea el estado igual que clickear el toggle
...
function NeverLose:CreateWindow(Config)
	...
	-- UICorner removido para esquinas cuadradas
	...
	-- Guardar referencia para poder cambiar el logo en tiempo real
	...
	-- Suscribirse al AccentColor signal para las líneas y borde del menú
	NeverLose:AddSignal(NeverLose.AccentColorSignal:Connect(function()
		LineFrame.BackgroundColor3 = NeverLose.AccentColor
		LineFrame_3.BackgroundColor3 = NeverLose.AccentColor
		UIStroke.Color = NeverLose.AccentColor
	...
	function Window:AddTab(Config)
		...
		-- Usar el nuevo sistema de iconos que soporta asset IDs
		...
		NeverLose:AddSignal(ActualIcon:GetPropertyChangedSignal(ActualIcon:IsA("ImageLabel") and 'ImageTransparency' or 'TextTransparency'):Connect(LPH_NO_VIRTUALIZE(function()
			local transparency = ActualIcon:IsA("ImageLabel") and ActualIcon.ImageTransparency or ActualIcon.TextTransparency
			if transparency > 0.4 then
				UIListLayout.Parent = nil;
		...
		-- Suscribirse al AccentColor signal para actualizar tab activo en tiempo real
		NeverLose:AddSignal(NeverLose.AccentColorSignal:Connect(function()
			if Tab.Signal:GetValue() then
				TabButton.BackgroundColor3 = NeverLose.AccentColor
			end
			TabIcon.TextColor3 = NeverLose.AccentColor
		end));
	...
	function Window:_InitConfig()
		...
		function ConfigLib:RefreshConfig()
			...
			local autoload_path = Window.ConfigFolder.."/autoload.txt"
			...
			pcall(function()
				for i,v in next , listfiles(Window.ConfigFolder) do
					...
					if name and name ~= "autoload.txt" then
			...
			for i,ConfigNameStr in next , ConfigList do
				...
				Icon_3.ZIndex = 153
				Icon_3.FontFace = NeverLose.BuiltInBold
				Icon_3.Text = "star"
				if is_currently_autoloaded then
					Icon_3.TextColor3 = Color3.fromRGB(244, 210, 90)
					Icon_3.TextTransparency = 0.1
				else
					Icon_3.TextColor3 = Color3.fromRGB(223, 223, 223)
					Icon_3.TextTransparency = 0.6
				end
				...
				local Render = LPH_NO_VIRTUALIZE(function(rst)
					if rst then
						...
						NeverLose.PlayAnimate(Icon_3,SlowyTween,{
							TextTransparency = is_currently_autoloaded and 0.1 or 0.6
						})
				...
				local deleter,signal = NeverLose:CreateInput(DeleteConfig,function()
					pcall(function() delfile(Window.ConfigFolder..'/'..ConfigNameStr); end);
				...
				local _,load_signal = NeverLose:CreateInput(LoadConfig,function()
					...
					if isfile(path) then
						local data;
						pcall(function() data = readfile(path); end);
				...
				local _,autoload_signal = NeverLose:CreateInput(AutoLoadConfig,function()
					local autoload_path = Window.ConfigFolder.."/autoload.txt"
					if is_currently_autoloaded then
						pcall(function() delfile(autoload_path); end);
						Logging.new("star",'Disabled AutoLoad for '..tostring(ConfigNameStr),3.5)
					else
						pcall(function() writefile(autoload_path, ConfigNameStr); end);
						Logging.new("star",'Set '..tostring(ConfigNameStr)..' as AutoLoad',3.5)
					end
				...
				table.insert(ConfigLib.Signals , AutoLoadConfig.MouseEnter:Connect(LPH_NO_VIRTUALIZE(function()
					if is_currently_autoloaded then
						NeverLose.PlayAnimate(Icon_3,SlowyTween,{
							TextTransparency = 0.3,
							TextColor3 = Color3.fromRGB(244, 210, 90)
						})
					else
						NeverLose.PlayAnimate(Icon_3,SlowyTween,{
							TextTransparency = 0.2,
				...
				table.insert(ConfigLib.Signals , AutoLoadConfig.MouseLeave:Connect(LPH_NO_VIRTUALIZE(function()
					if is_currently_autoloaded then
						NeverLose.PlayAnimate(Icon_3,SlowyTween,{
		...
		local hover_write = NeverLose:CreateInput(ConfigIcon,function()
			...
			if isfile(path) then
				pcall(function() writefile(Window.ConfigFolder..'/'..(ConfigLib.SelectedConfig or "Default"),ConfigLib:GetData()); end);
		...
		local mv = NeverLose:CreateInput(LoadConfig , function()
			...
			if cfg_name and cfg_name:byte() and not cfg_name:find('/',1,true) and not cfg_name:find('\\',1,true) then
				...
				pcall(function() writefile(Window.ConfigFolder..'/'..cfg_name,ConfigLib:GetData()); end);
				...
				if name ~= "" then
					...
					if isfile(path) then
						...
						if load_success and data then
							...
							Logging.new("star", 'Autoloaded configuration: '..tostring(name), 3.5)
						end
					end
				end
			end
